...
async function sendViaBrevo({ apiKey, fromEmail, fromName, toAddresses, ccAddresses, subject, html }) {
  const res = await fetch(BREVO_API_URL, {
    ...
    body: JSON.stringify({
      sender: { email: fromEmail, name: fromName || 'Account Health Alerts' },
      to: toAddresses,
...
function buildTransportForRecipients(toEmails, ccEmails) {
  ...
  const fromName = match ? match[1].trim() : 'Account Health Alerts';
...
export async function sendPodDigestEmail({ podName, to, cc, clientReports }) {
  ...
  const subject = `[Daily Digest] ${escapedPodName} — ${totalPendingJobs} Pending L/XL/XXL Job${totalPendingJobs !== 1 ? 's' : ''} (${todayStr})`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #f8fafc;">
      <h2 style="color: #2563eb; margin-top: 0; font-size: 20px;">📋 Daily Digest — ${escapedPodName}</h2>
      ...
      <p style="font-size: 11px; color: #94a3b8; text-align: center; margin: 0;">
        This is an automated daily digest generated by the Account Health Tracker Dashboard.
      </p>
    </div>
  `;
